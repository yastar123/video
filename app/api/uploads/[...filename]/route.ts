...
export async function GET(
  ...
  if (!existsSync(absolutePath)) {
    ...
    if (extension === '.ts' || extension === '.m3u8') {
      ...
      // For .m3u8 files, create a proper HLS playlist
      if (extension === '.m3u8') {
        ...
        if (videoFile && existsSync(videoFile)) {
          // Create a valid HLS playlist that points to the video file
...
#EXT-X-ENDLIST`
        ...
        // If no video file found, create a simple HLS playlist with a dummy segment
        const dummyPlaylist = `#EXTM3U
#EXT-X-VERSION:3
#EXT-X-MEDIA-SEQUENCE:0
#EXT-X-TARGETDURATION:10
#EXTINF:10.0,
...
#EXT-X-ENDLIST`
        ...
        return new Response(dummyPlaylist, {
          headers: {
            'Content-Type': 'application/vnd.apple.mpegurl',
            'Content-Length': dummyPlaylist.length.toString(),
            'Cache-Control': 'no-cache',
            'Access-Control-Allow-Origin': '*',
            'Access-Control-Allow-Methods': 'GET, POST, PUT, DELETE, OPTIONS',
            'Access-Control-Allow-Headers': 'X-Requested-With, Content-Type, Authorization'
      ...
      // For .ts segments, return a small dummy video segment
      if (extension === '.ts') {
        // Create a minimal valid TS segment (just a few bytes)
        const dummyTsSegment = Buffer.from([
          0x47, 0x40, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00, // TS header
          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Padding
          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00  // More padding
        ])
        
        return new Response(dummyTsSegment, {
          headers: {
            'Content-Type': 'video/mp2t',
            'Content-Length': dummyTsSegment.length.toString(),
            'Cache-Control': 'no-cache',
            'Access-Control-Allow-Origin': '*',
            'Access-Control-Allow-Methods': 'GET, POST, PUT, DELETE, OPTIONS',
            'Access-Control-Allow-Headers': 'X-Requested-With, Content-Type, Authorization'
